...
ggplot(movies_budget, aes(x = log(budget + 1), y = imdb_score)) +
  ...
  labs(title = "IMDb ocjena po budžetu filma",
       ...
       y = "IMDb ocjena")
```
Prema grafu i koeficijentu korelacije možemo zaključiti da nema linearne ovisnosti između ocjene i budžeta.
...
ggplot(movies_gross, aes(x = log(gross + 1), y = imdb_score)) +
  ...
  labs(title = "IMDB ocjena po zaradi od ulaznica",
       ...
       y = "IMDb ocjena")
```
Iako je u ovom slučaju koeficijent korelacije veći nego kod budžeta, ne možemo zaključiti postojanje linearne ovisnosti.
...
```{r}
par(mfrow = c(1,2))
hist(residuals(lm1), breaks = 40, main = "Residuals histogram")
...
```{r}
par(mfrow = c(1,2))
hist(residuals(lm2), breaks = 40, main = "Residuals histogram")
qqnorm(residuals(lm1))
qqline(residuals(lm1))
